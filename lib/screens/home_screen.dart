...
class _HomeScreenState extends State<HomeScreen> {
  ...
  Future<void> _loadData() async {
    try {
      ...
      setState(() {
        _mainPackages = services
            .where((s) => s['serviceType'] == 'Package')
            .toList();
  ...
  Future<void> _saveVoucher(Map<String, dynamic> voucherData) async {
      ...
      if (result['error'] != null) {
        if (result['error'] == 'Unauthorized') {
          ScaffoldMessenger.of(context).showSnackBar(
            const SnackBar(
              content: Text('Vui lòng đăng nhập để lưu mã giảm giá!'),
            ),
          );
        } else {
    ...
    if (exists) {
      ...
      ScaffoldMessenger.of(
        context,
      ).showSnackBar(const SnackBar(content: Text('Bạn đã lưu mã này rồi!')));
      return;
    }
                  ...
                  : SizedBox(
                      ...
                      child: ListView.builder(
                        ...
                        itemBuilder: (context, index) {
                          ...
                          if (pkg['prices'] != null &&
                              (pkg['prices'] as List).isNotEmpty) {
                            displayPrice = (pkg['prices'][0]['price'] as num)
                                .toInt();
                            displayDuration =
                                (pkg['prices'][0]['durationMinutes'] as num)
                                    .toInt();
                          }
  ...
  Widget _buildServicePackageCard({
    ...
    return Container(
      ...
      decoration: BoxDecoration(
        ...
        borderRadius: BorderRadius.circular(16),
        border: Border.all(
          color: isPopular ? const Color(0xFF0F2050) : Colors.grey.shade200,
          width: isPopular ? 2 : 1,
        ),
        boxShadow: [
          BoxShadow(
          ...
          children: [
            Padding(
              ...
              child: Column(
                ...
                children: [
                  Icon(
                    title.toLowerCase().contains('cao cấp')
                        ? Icons.diamond
                        : (title.toLowerCase().contains('nâng cao')
                              ? Icons.waves
                              : Icons.water_drop),
                    color: const Color(0xFF0F2050),
                    size: 24,
